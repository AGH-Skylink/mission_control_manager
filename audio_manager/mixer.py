...
class AudioMixer:
    def __init__(self, num_channels: int = 4, num_tablets: int = 16) -> None:
        self.num_channels = num_channels
        self.num_tablets = num_tablets
        self.config = MixConfig()

        self._tablet_in = {
            tid: np.zeros(FRAME_SIZE, dtype=np.float32)
            for tid in range(1, num_tablets + 1)
        }
        self._tablet_out = {
            tid: np.zeros(FRAME_SIZE, dtype=np.float32)
            for tid in range(1, num_tablets + 1)
        ...
        self._chan_in = {
            ch: np.zeros(FRAME_SIZE, dtype=np.float32)
    ...
    def set_uniform_routing(self, gain_db: float = -12.0) -> None:
        g = 10 ** (gain_db / 20.0)
        ...
        for tid in range(1, self.num_tablets + 1):
            self.config.downlink[tid] = {
                ch: g for ch in range(1, self.num_channels + 1)
            }

    @staticmethod
    def _soft_limit(x: np.ndarray, knee: float = 0.9) -> np.ndarray:
        y = np.tanh(x / knee) * knee
        return y

    def _update_rms(self, kind: str, key: int, frame: np.ndarray) -> None:
        rms = float(np.sqrt(np.mean(np.square(frame))))

        if kind == "tablet":
            last = self.vu.tablet_rms.get(key, 0.0)
            self.vu.tablet_rms[key] = last * (1.0 - self._ema_alpha) + rms * self._ema_alpha
        else:
            last = self.vu.channel_rms.get(key, 0.0)
            self.vu.channel_rms[key] = last * (1.0 - self._ema_alpha) + rms * self._ema_alpha

    def vu_levels_db(self) -> Dict[str, Dict[int, float]]:
        return {
            "tablets": {
                tid: float(dbfs(v * 1.0000001)) for tid, v in self.vu.tablet_rms.items()
            },
            "channels": {
                ch: float(dbfs(v * 1.0000001))
                for ch, v in self.vu.channel_rms.items()
            },
        }
    ...
    def set_uplink_matrix(self, matrix: Dict[int, Dict[int, float]]) -> None:
        self.config.uplink = {
            int(ch): {int(tid): float(gain) for tid, gain in row.items()}
            for ch, row in matrix.items()
        }

    def set_downlink_matrix(self, matrix: Dict[int, Dict[int, float]]) -> None:
        self.config.downlink = {
            int(tid): {int(ch): float(gain) for ch, gain in row.items()}
            for tid, row in matrix.items()
        }

    def set_tablet_mute(self, tid: int, mute: bool) -> None:
        self.config.tablet_mute[int(tid)] = bool(mute)

    def set_channel_mute(self, ch: int, mute: bool) -> None:
        self.config.channel_mute[int(ch)] = bool(mute)

    def push_tablet_frame(self, tid: int, pcm_i16: np.ndarray) -> None:
        self._tablet_in[int(tid)] = self._to_float(pcm_i16)

    def pull_tablet_frame(self, tid: int) -> np.ndarray:
        return self._from_float(self._tablet_out[int(tid)])

    def push_channel_frame(self, ch: int, pcm_i16: np.ndarray) -> None:
        self._chan_in[int(ch)] = self._to_float(pcm_i16)

    def pull_channel_frame(self, ch: int) -> np.ndarray:
        return self._from_float(self._chan_out[int(ch)])
    ...
    def step(self) -> None:
        for ch, mapping in self.config.uplink.items():
            ...
            sum_g = sum(
                ...
                if not self.config.tablet_mute.get(tid, False)
            )
            headroom_lin = 10.0 ** (-self.config.headroom_db / 20.0)
            norm = 1.0 if sum_g <= 0.0 else min(1.0, headroom_lin / max(sum_g, 1e-9))
        ...
        for tid, mapping in self.config.downlink.items():
            ...
            sum_g = sum(
                ...
                if not self.config.channel_mute.get(ch, False)
            )
            headroom_lin = 10.0 ** (-self.config.headroom_db / 20.0)
            norm = 1.0 if sum_g <= 0.0 else min(1.0, headroom_lin / max(sum_g, 1e-9))
